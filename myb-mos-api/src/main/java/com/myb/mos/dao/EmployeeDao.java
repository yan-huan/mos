...
public class EmployeeDao extends MySqlCommandBase
{
    ...
    {
        String sql="INSERT INTO "+getTableName()+" (empId,parentId,empName,account,empCode,dutyId,parentName,depName,cityPermission,isUsed,cityId,createTime,userName,passWord,status,ran)VALUES(@{empId},@{parentId},@{empName},@{account},@{empCode},@{dutyId},@{parentName},@{depName},@{cityPermission},@{isUsed},@{cityId},@{createTime},@{userName},@{passWord},@{status},@{ran})";
    ...
    {
        String sql="UPDATE "+getTableName()+" set parentId=@{parentId},empName=@{empName},account=@{account},empCode=@{empCode},dutyId=@{dutyId},parentName=@{parentName},depName=@{depName},cityPermission=@{cityPermission},isUsed=@{isUsed},cityId=@{cityId},createTime=@{createTime},userName=@{userName},passWord=@{passWord},status=@{status},ran=@{ran} WHERE empId=@{empId}";
        SqlParameter sqlParameter=new SqlParameter();
        sqlParameter.add(EmployeeMapping.EmpId, employee.getEmpId());
        sqlParameter.add(EmployeeMapping.ParentId, employee.getParentId());
        sqlParameter.add(EmployeeMapping.EmpName, employee.getEmpName());

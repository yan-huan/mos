		...
		$(function(){
			$("#searchBeautiful").click(function(){
				...
				$.ajax({
	    				...
	    				url:"../Account/findByBeautifulId.jhtml?finance=1",
	    				success:function(data){
	    					console.info(data);
	    					if(data != "-1"){
	    						$("#searchContent").html("");
								...
								<c:when test="${finance eq 1 }">
										<c:forEach items="${openAccountPageList.content}" var="openAccountPageList">
											...
											<tr>
												...
												<c:if test="${finance eq 1}">
												<td><a href="../Account/index.jhtml?type=2&isqudao=isqudao&finance=1&accountId=${openAccountPageList.accountId}" target="view_window">${openAccountPageList.beautifulyard}</a></td>
												</c:if>
												...
												<td>
													...
													<a href="../channelInfo/findAllChannelInfo.jhtml?accountId=${openAccountPageList.accountId}&finance=${finance}" class="btn btn-xs btn-info">审核</a>
													<a href="../contract/index.jhtml?pageIndex=1&accountId=${openAccountPageList.accountId}">查看历史合同</a>
													</c:if>

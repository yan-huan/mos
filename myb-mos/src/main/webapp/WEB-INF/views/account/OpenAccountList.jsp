...
<html lang="en"><head>
		...
		function pageForword(pageIndex){
			...
			$.ajax({
				...
				data:"",
				datatype:"html",
				url:"../Account/showListOpenAccount.jhtml?finance="+finance+"&pageIndex="+pageIndex,
							...
							<form id="yanzheng">
								<table class="table-nobordered">
									<tbody><tr>
										...
										<td><img alt="" src="../images/deng.png">延期或者修改信息过</td>
									</tr>
								</tbody>
								</table>
								</form>
							...
							<c:choose>
								<c:when test="${finance eq 1}">
										<c:forEach items="${openAccountPageList.content}" var="openAccountPageList">
											...
											<tr>
												<td>
												<a href="../Account/index.jhtml?type=2&finance=1&accountId=${openAccountPageList.accountId}" target="view_window">${openAccountPageList.beautifulyard}</a>
								...
								<c:otherwise>
									<c:forEach items="${openAccountPageList.content}" var="openAccountPageList">
											...
											<tr>
												<td>
													<a href="../Account/index.jhtml?type=2&accountId=${openAccountPageList.accountId}">${openAccountPageList.beautifulyard}</a>
													<c:if test="${openAccountPageList.isException eq 1}"><!--销售助理看审批状态  -->
														...
														<!-- 	//处理状态 -->
															...
															<c:if test="${openAccountPageList.ispass eq 2}">
																<font class="text-status-danger" size="3">
																	未通过
																</font>
															</c:if>
